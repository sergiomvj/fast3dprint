...
# Mount the generated_models directory to serve static files (3D Models)
...
# Update: Serve Frontend (Single Container Deployment Strategy)
# Check if "static_ui" (copied from frontend build) exists
if os.path.exists("static_ui"):
    app.mount("/assets", StaticFiles(directory="static_ui/assets"), name="assets")
    
    @app.get("/{full_path:path}")
    async def serve_spa(full_path: str):
        # Serve index.html for any known non-api route to let React Router handle it
        if full_path.startswith("static/") or full_path.startswith("assets/"):
            return FileResponse(f"static_ui/{full_path}")
